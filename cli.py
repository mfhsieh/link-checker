...
def merge_and_validate_crawler_config(
    ...
    for key in list(crawler_config.keys()):
        if key not in allowed_crawler_keys:
            logging.warning(
                "個別設定 config.yaml 不允許覆寫或設定 crawler.%s，此設定將被忽略。",
                key,
            )
    ...
    global_ignore_extensions: list[str] = (
        global_crawler_config.get("ignore_extensions") or []
    )
    ...
    if global_ignore_extensions or local_ignore_extensions:
        crawler_config["ignore_extensions"] = list(
            set(global_ignore_extensions + local_ignore_extensions)
        )
    ...
    if global_ignore_regexes or local_ignore_regexes:
        crawler_config["ignore_regexes"] = list(
            set(global_ignore_regexes + local_ignore_regexes)
        )
    ...
    global_approved_domains: list[str] = (
        global_crawler_config.get("approved_domains") or []
    )
    ...
    if global_approved_domains or local_approved_domains:
        crawler_config["approved_domains"] = list(
            set(global_approved_domains + local_approved_domains)
        )
    else:
    ...
    if global_ssl_exempt or local_ssl_exempt:
        crawler_config["ssl_exempt_domains"] = list(
            set(global_ssl_exempt + local_ssl_exempt)
        )
    else:
    ...
    if crawler_config["timeout"] < min_timeout:
        logging.warning(
            "個別設定的 timeout (%s) 小於全域最小值 (%s)，將強制套用全域最小值。",
        ...
        logging.warning(
            "個別設定的 timeout (%s) 大於全域最大值 (%s)，將強制套用全域最大值。",
            crawler_config["timeout"],
            max_timeout,
        )
    ...
    if crawler_config["delay"] < min_delay:
        logging.warning(
            "個別設定的 delay (%s) 小於全域最小值 (%s)，將強制套用全域最小值。",
            crawler_config["delay"],
            min_delay,
        )
        crawler_config["delay"] = min_delay
    elif crawler_config["delay"] > max_delay:
        logging.warning(
            "個別設定的 delay (%s) 大於全域最大值 (%s)，將強制套用全域最大值。",
            crawler_config["delay"],
            max_delay,
        )
    ...
    if crawler_config["retries"] < min_retries:
        logging.warning(
            "個別設定的 retries (%s) 小於全域最小值 (%s)，將強制套用全域最小值。",
            crawler_config["retries"],
            min_retries,
        )
        crawler_config["retries"] = min_retries
    elif crawler_config["retries"] > max_retries:
        logging.warning(
            "個別設定的 retries (%s) 大於全域最大值 (%s)，將強制套用全域最大值。",
            crawler_config["retries"],
            max_retries,
        )
...
def main() -> None:
    ...
    if args.list_jobs:
        ...
        if args.json:
            ...
            print("\n=== 爬蟲任務列表 ===")
            print(
                f"{'Job ID':<38} | {'User ID':<20} | {'Status':<10} | {'Created At':<20} | {'Start URL'}"  # pylint: disable=line-too-long
            )
            print("-" * 120)
            for j in jobs:
                uid = j.get("user_id") or "N/A"
                print(
                    f"{j['id']:<38} | {uid:<20} | {j['status']:<10} | {j['created_at']:<20} | {j['start_url']}"  # pylint: disable=line-too-long
                )
            print("====================\n")
        return

...
export class LinkDataTable extends HTMLElement {
    ...
    _renderPagination() {
        ...
        btnFirstEl.className = "btn btn-secondary btn-sm";
        btnFirstEl.textContent = "«";
        btnFirstEl.title = "第一頁";
        btnFirstEl.disabled = current <= 1;
        btnFirstEl.addEventListener("click", () => {
            if (current > 1) this._dispatchPageChange(1);
        });

        const btnPrevEl = document.createElement("button");
        btnPrevEl.className = "btn btn-secondary btn-sm";
        btnPrevEl.textContent = "‹";
        btnPrevEl.title = "上一頁";
        ...
        btnNextEl.className = "btn btn-secondary btn-sm";
        btnNextEl.textContent = "›";
        btnNextEl.title = "下一頁";
        ...
        btnLastEl.className = "btn btn-secondary btn-sm";
        btnLastEl.textContent = "»";
        btnLastEl.title = "最後一頁";
        btnLastEl.disabled = current >= total;
        btnLastEl.addEventListener("click", () => {
            if (current < total) this._dispatchPageChange(total);
        });

        this._paginationEl.appendChild(btnFirstEl);
        this._paginationEl.appendChild(btnPrevEl);
        this._paginationEl.appendChild(infoEl);
        this._paginationEl.appendChild(btnNextEl);
        this._paginationEl.appendChild(btnLastEl);
    }

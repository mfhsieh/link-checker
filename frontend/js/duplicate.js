...
export async function initDuplicatePage() {
    ...
    try {
        ...
        jobs.forEach(j => {
            const statusStr = j.status === 'completed' ? '已完成' : (j.status === 'paused' ? '已暫停' : (j.status === 'pending' ? '等待中' : (j.status === 'error' ? '異常' : '執行中')));
            jobSelectEl.appendChild(new Option(`${j.id.substring(0, 8)}... - ${j.start_url} [${statusStr}]`, j.id));
        });

...
> [!WARNING]
> **Production 執行注意事項**：當伺服器處於高負載狀態時，透過終端機 (CLI) 啟動的大型爬蟲任務，往往比透過 Web UI 啟動的任務更容易遭到系統強制中斷（主要原因為 OOM Killer 的進程分片優先級，或 Systemd 的連線登出清理機制）。若必須在遠端主機上使用 CLI 執行長時任務，強烈建議**使用 `nohup` 並將輸出導向到 `/dev/null`**，或是透過 `tmux` / `screen` 等虛擬終端機執行，以降低意外中斷的風險。
> 
> 安全背景執行範例：`nohup python cli.py --resume <job_id> > /dev/null 2>&1 &`
...
```
伺服器啟動後，終端機會印出準備就緒的提示（如 `👉 請打開瀏覽器，前往： http://127.0.0.1:8000`）。系統預設監聽在 `127.0.0.1:8000`，您可透過終端機的標準輸出檢視存取紀錄，按下 `Ctrl+C` 即可安全關閉伺服器。
...
## 9. 資料庫與系統磁碟空間維護 (Disk Cleanup)

為了解決長期執行爬蟲導致的資料庫膨脹與系統暫存檔堆積問題，系統提供了一支綜合維護腳本 (`scripts/cleanup_disk.sh`)。這支腳本整合了作業系統層級的清理與 PostgreSQL 的無鎖定最佳化機制。
...
### 腳本執行內容說明
1. **系統套件清理**：自動移除 APT 孤立套件、清除舊版 Snap 暫存檔，釋放系統底層空間。
2. **日誌瘦身**：清理龐大的 `systemd` 系統日誌，僅保留最近 28 天的紀錄。
3. **容量成效比對**：執行清理前後，會自動印出根目錄 (`/`) 與資料庫 (`crawler_db`, `auth_db`) 的佔用大小比對。
4. **兩階段無鎖定空間重組 (`pg_repack`)**：
   - 腳本會自動檢查並調用 `pg_repack` 進行資料庫最佳化（無須停止爬蟲）。
   - 針對龐大的 `crawler_db`，腳本採用「化整為零」的智慧策略：依資料表大小（由小到大）逐一進行，並採用「先重組索引、再重組資料表」的兩階段無鎖重組，大幅降低硬碟剩餘空間的要求，避免觸發空間不足的崩潰。

---

## 10. MCP Server 整合 (AI 助理代理)

...
*   **`test/test_job_diff.py`**：
    *   **職責**：針對任務歷史差異比對服務 (Job Diff Engine) 進行核心演算法與狀態分類精確度驗證。
    *   **主要測試範疇**：
        1. **統計精確度**：驗證外部連結比對（IP 變更、降級、復原、持續失敗、新增/移除）與內部網頁比對（內部降級、內部復原、內部持續失敗、新增/移除頁面）的計算完全精確。
        2. **資料庫層級整合**：在 SQLite 記憶體資料庫中建立模擬任務、外部連結與內部網頁紀錄，驗證 `get_job_diff` 回傳之摘要數量與明細項目是否正確寫入 `job_diff_results` 與 `job_diff_items` 資料表。
    *   **狀態管理**：在檔案內部採用專屬 SQLite 記憶體資料庫 (`sqlite:///:memory:`) 進行極速單元測試，無實體檔案殘留。
...
*   **`test/test_skipped_head_check.py`**：
    *   **職責**：針對被忽略內部連結之輕量死檔探測功能進行單元測試與功能驗證。
    *   **主要測試範疇**：
        1. **探測存活 (200 OK)**：驗證當啟用 `check_skipped_links` 且連結符合忽略規則時，發送 GET 串流請求，在取得 HTTP 標頭並確認 200 OK 後立即中斷串流（不下載 Body），最終狀態應為 `skip`。

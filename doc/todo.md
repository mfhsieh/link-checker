...
1. **重構任務比對架構：實作比對結果持久化 (Materialized Diff Table)**
   ...
   * **規劃方案**：捨棄一次性 JSON 回傳，改採「比對結果持久化」。當使用者首次請求比對時，由後端執行算出結果並寫入實體資料表（如 `job_diff_results`），前端後續的統計數字、分頁與匯出皆直接從該表做分頁查詢。為避免佔用過多空間，搭配**「自動清理超過 14 天未存取的比對紀錄」**的定時任務即可，無需增加使用者「手動釘選」的認知負擔。
   * **狀態**：**待排程（Pending）**。

...
#### 3. 針對 crawler/core.py 引入 Strategy Pattern 的設計模式
...
#### 5. 擴充比對任務 (Job Diff) 支援內部連結與診斷邏輯優化
...
#### 8. 建立 MCP Server 以監控遠端 Production 任務狀態
...
#### 9. 前端程式碼重構：導入 MVC 或 Web Components 模組化封裝
...
#### 11. 透過事件機制實作全域設定與快取自動更新 (Cache Invalidation)
...
#### 12. 透過事件機制實作統計與報表聚合 (Metrics Aggregation)
...
#### 13. 實作全局 API 速率限制 (Global Rate Limiting)
...
#### 14. 主爬行迴圈與健康診斷之非同步解耦架構 (Async Distributed Architecture)
...
#### 15. CLI 支援匯出內部紀錄之狀態篩選 (export-internal filter)
...
#### 16. CSRF Token 與 Session 綁定（R2-02）

...
本系統的爬蟲引擎設計具備高度彈性，共提供 **22 個** 可供配置的參數。其中 19 個參數能在建立「個別爬蟲任務 (`job/*.yaml`)」時精確指定，另外 3 個為系統級核心防護限制，僅能於「全域設定檔 (`config/config_global.yaml`)」中配置。
...
* **`target_domains`** (字串陣列)：允許爬蟲深入遍歷（並執行內部網頁診斷）的目標網域清單。例如：`["example.com", "blog.example.com"]`。
* **`trusted_domains`** (字串陣列 | 選填)：被視為自家系統或可信任的網域清單。用來判斷抓到的連結是不是「外部連結」。
...
* **`timeout`** (整數)：抓取內部完整網頁（含 HTML 解析與死鏈診斷）的總超時時間 (預設 30 秒)。
* **`connect_timeout`** (浮點數)：建立 TCP 連線的超時時間，專門防禦惡意不回應的伺服器 (預設 5.0 秒)。
* **`external_check_timeout`** (浮點數)：針對外部連結進行獨立存活探測 (HEAD/GET) 的總超時時間 (預設 10.0 秒)。
* **`delay`** (浮點數)：每次發送 HTTP 請求前的等待延遲時間，避免對目標伺服器造成 DDoS (預設 3.0 秒)。
* **`jitter_ratio`** (浮點數 | **僅限全域設定**)：請求延遲與退避的隨機抖動比例，用以防範行為分析 (預設 0.2，代表 ±20% 抖動)。
...
* **`max_content_length`** (整數 | **僅限全域設定**)：最大允許下載的網頁容量 (Bytes)。這是保護系統記憶體的硬性限制 (防禦 OOM 崩潰)，個別任務無法覆寫 (預設 10MB)。
* **`max_redirects`** (整數 | **僅限全域設定**)：HTTP 重導向追蹤次數上限。避免陷入惡意轉址迴圈 (Redirect Loop)，個別任務無法覆寫 (預設 10 次)。

---

## 4. 過濾與排除 (節省頻寬與時間)

告訴爬蟲什麼連結與檔案「不要」碰，以及是否對其進行存活確認。
...
* **`check_skipped_links`** (布林值)：是否啟用對被忽略內部連結的存活探測 (預設為 `true`)。開啟後，符合略過規則的內部連結不會下載檔案內容，但仍會發送輕量 HTTP 請求以確認連結是否正常。
...
## 6. 配置診斷與查詢 (Configuration Diagnostic)

任務啟動時，系統會將合併並套用全域防呆限制後的完整參數儲存為快照 (Config Snapshot)。
為了方便開發與除錯，您可以透過 **MCP 伺服器** (使用 `get_job_config` 工具) 直接查詢執行中任務的最終生效參數值，藉此快速釐清爬蟲行為是否受限於安全防護機制。
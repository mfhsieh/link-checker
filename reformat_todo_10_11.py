...
for i in range(len(item_10)):
    if "* **狀態**：" in item_10[i]:
        item_10[i] = "* **狀態**：**已擱置（Dropped）** - 原因：目前快取僅針對靜態的歷史任務診斷結果，並不會因為全域設定或使用者權限異動而導致資料不同步，因此無需實作複雜的快取更新事件。\n"

for i in range(len(item_11)):
    if "* **狀態**：" in item_11[i]:
        item_11[i] = "* **狀態**：**已擱置（Dropped）** - 原因：系統目前主要需求為「針對單一任務的診斷報告」，並無跨任務的大型數據統計看板需求。直接修改核心或增加獨立報表服務屬過度設計。\n"

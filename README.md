...
- **v1.9.8 (2026-07-31)**:
  - **任務比對 (Job Diff) 穩定性與體驗升級**：
    - 提升系統穩定度：解決當多人同時查詢相同的「任務比對」結果時，系統偶爾會當機報錯 (500 Error) 的問題。
    - 改善表格閱讀體驗：統一「比對報表」與「任務詳情」的排版風格，將狀態碼、網頁數量等欄位全面調整為置中對齊。
  - **測試穩定度優化**：修復了自動化測試時，因頁面跳轉太快而導致的執行中斷問題。
- **v1.9.7 (2026-07-29)**:
  ...
  - 爬蟲核心引擎完全解耦：移除 `crawler` 對 `backend.events` 的引用，改以事件 Callback 注入達成零依賴。

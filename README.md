...
- **v1.9.7 (2026-07-27)**:
  - 新增「探測略過連結」功能，遇到像 PDF、ZIP 等被略過的內部連結時，只發送輕量請求檢查檔案是否存在，不下載檔案內容以節省網路流量。
  - 調整網頁操作介面：在新增任務與管理員頁面加上啟用開關、複製舊任務時自動相容，並在輸入網址時自動切除開頭的 `www.` 以防止填寫出錯。
  - 系統安全性與穩定度強化：隱藏伺服器錯誤細節避免敏感資訊外洩、優化資料庫連線防護防止瞬斷時任務卡死，並加入無窮迴圈防禦機制。
  - 網頁讀取與紀錄優化：網頁更新時自動刷新快取無需重啟服務、加速統計數據查詢，並讓系統紀錄能夠清晰標示對應的任務。
  - 完整測試與說明同步：補充自動化測試確保各種複雜網路環境下的穩定度，並同步更新專案規格與說明文件。
- **v1.9.6 (2026-07-18)**: 

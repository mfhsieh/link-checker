...
- **v1.9.7 (2026-07-27)**:
  - 新增「探測略過連結」功能（僅輕量檢查檔案存活不下載內容），並於介面加入設定開關。
  - 網頁操作防呆優化：輸入網址時自動切除開頭 `www.` 前綴，防止目標網域填寫偏差。
  - 系統資安與穩定度優化：遮蔽伺服器錯誤細節防洩漏、提升資料庫連線容錯、防止無窮迴圈並實作網頁快取自動刷新。
  - 補齊核心降級路徑之自動化測試，並同步更新相關系統規格與說明文件。
- **v1.9.6 (2026-07-18)**: 

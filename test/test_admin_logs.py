...
# 測試用 SQLite DSN
TEST_AUTH_DB_URL = "sqlite:///tmp/auth_test.db"
...
# 模擬的管理員物件
mock_admin = User(id="admin-id", email="admin@test.com", role="admin", status="active")
...
class TestAdminLogs(unittest.TestCase):
    ...
    def test_user_status_changed_logging(self):
        payload = {
            "status": "suspended",
            "role": "admin"
        }
    ...
    def test_logs_date_filtering(self):
        db = TestingSessionLocal()
        # 建立幾個不同時間點的日誌
        now = datetime.now()

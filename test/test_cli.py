...
def test_cli_full_flow() -> None:
    ...
    test_crawler_config = merge_and_validate_crawler_config(
        cast(dict[str, object], {"crawler": {"ignore_extensions": [".pdf", ".jpg"], "max_depth": 3}}),
        cast(dict[str, object], global_cfg),
    )
    assert set(cast(list[str], test_crawler_config["ignore_extensions"])) == {".pdf", ".jpg"}
    merged = merge_and_validate_crawler_config(cast(dict[str, object], local_cfg), cast(dict[str, object], global_cfg))

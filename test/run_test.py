...
# 將根目錄加入 sys.path 以便匯入 crawler 模組
...
def run_test():
    ...
    try:
        ...
        assert ext_dict[google_url]['status_code'] == 200, f"Google status code should be 200, got {ext_dict[google_url]['status_code']}"
        ...
        assert ext_dict[dns_fail_url]['ip'] is None, f"DNS fail link IP should be None, got {ext_dict[dns_fail_url]['ip']}"
        assert ext_dict[dns_fail_url]['status_code'] is None, f"DNS fail link status code should be None, got {ext_dict[dns_fail_url]['status_code']}"
        ...
        assert ext_dict[neverssl_url]['is_secure'] == 0, f"neverssl.com is_secure should be 0 (False), got {ext_dict[neverssl_url]['is_secure']}"
        ...
        export_cmd = [sys.executable, "cli.py", "--export", job_id, "--json", "--group", "--output", export_file]
        ...
        for item in export_data:
            if item['target_url'] == status_404_url:
                ...
                assert item['occurrence_count'] == 2, f"Expected 404 occurrence count to be 2, got {item['occurrence_count']}"
                assert len(item['source_urls']) == 2, "Expected 2 source urls for 404 link"
                assert "http://localhost:8000/index.html" in item['source_urls'], "Missing index.html source"
                assert "http://localhost:8000/page2.html" in item['source_urls'], "Missing page2.html source"
        ...
        export_dead_cmd = [sys.executable, "cli.py", "--export", job_id, "--json", "--filter", "dead", "--output", dead_file]
        ...
        assert dead_data[0]['target_url'] == "https://this-dns-does-not-exist-at-all-123456789.com", "Dead link target mismatch"
        os.remove(dead_file)
        
        # 測試 --filter broken
        broken_file = "tmp_broken.json"
        if os.path.exists(broken_file): os.remove(broken_file)
        export_broken_cmd = [sys.executable, "cli.py", "--export", job_id, "--json", "--filter", "broken", "--output", broken_file]
        ...
        if os.path.exists(unapproved_file): os.remove(unapproved_file)
        export_unapproved_cmd = [sys.executable, "cli.py", "--export", job_id, "--json", "--filter", "unapproved", "--output", unapproved_file]
        res_unapproved = subprocess.run(export_unapproved_cmd, capture_output=True, text=True)
        ...
        # 預期不屬於白名單的外連有 7 個
        assert len(unapproved_data) == 7, f"Expected 7 unapproved links, got {len(unapproved_data)}"
        ...
        # 等待背景任務結束 (協同暫停)
        try:
            stdout_bg, stderr_bg = bg_proc.communicate(timeout=10)
        ...
        assert rel_count == 0, f"relative-link.html (depth 2) should NOT exist in queue when max_depth=1, got {rel_count}"
        ...
        assert total_pages_crawled == 3, f"Total crawled pages should be exactly 3 when max_pages=3, got {total_pages_crawled}"
